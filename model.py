...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings):
        ...
        self.tag_embeddings = he_initializer((self.config.n_tag_ids, self.config.embed_size)).requires_grad_(True)
        self.deprel_embeddings = he_initializer((self.config.n_deprel_ids, self.config.embed_size)).requires_grad_(True)
        # *** END YOUR CODE ***
    ...
    def embedding_lookup(self, id_batch, n_ids, embedding_matrix):
        ...
        """
        # *** BEGIN YOUR CODE ***
        # convert id_batch to one-hot vectors
        one_hot = one_hot_float(id_batch, n_ids)  # shape = (B, N, n_ids)
        # multiply (B, N, n_ids) with (n_ids, embed_size) by reshaping the first to (B*N, n_ids)
        # then reshape the product (B*N, embed_size) to (B, N, embed_size)
        B = id_batch.size()[0]
        N = id_batch.size()[1]
        embedded_batch = torch.mm(one_hot.view(-1, n_ids), embedding_matrix).view(B, N, -1)
        # finally reshape to (B, N*embed_size)
    ...
    def get_concat_embeddings(self, word_id_batch, tag_id_batch,
        ...
        """
        # *** BEGIN YOUR CODE ***
        word_embed = self.embedding_lookup(word_id_batch, self.config.n_word_ids, self.word_embeddings) # (B, n*embed_size)
        ...
        x = torch.cat((word_embed, tag_embed, deprel_embed), 1)  # (B, N*embed_size)
        # *** END YOUR CODE ***
        return x
    ...
    def add_optimizer(self):
        ...
        optimizer = torch.optim.Adam(params, self.config.lr)
